...
public class RN_Calc_UI_T {
	...
	public RN_Calc_UI_T(RN_Calc rnc) {
		this.rnc = rnc;
		...
		JTextArea textAr = new JTextArea(getTeamText());
		...
		//scrollPane = new JScrollPane(textAr, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		scrollPane = new JScrollPane(jl, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		scrollPane.setPreferredSize(new Dimension(200, 200));
	    aboutMenu.setFont(tnr);
		aboutMenu.setMnemonic(KeyEvent.VK_A);
		aboutMenu.getAccessibleContext().setAccessibleDescription("About People Who developed the program");
		...
		JMenuItem userGuide = new JMenuItem("Guide");
		ImageIcon id = new ImageIcon("./popupFiles/Guide.png");
		JTextArea textSpace = new JTextArea(getGuide());
		JScrollPane si;
		textSpace.setEditable(false);
	    textSpace.setLineWrap(true);
		textSpace.setOpaque(false);
		textSpace.setFont(tnr);
		textSpace.setWrapStyleWord(true);
		si = new JScrollPane(textSpace, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		si.setPreferredSize(new Dimension(320, 240));
		aboutMenu.setMnemonic(KeyEvent.VK_G);
		aboutMenu.getAccessibleContext().setAccessibleDescription("A guide to use the calculator");
	...
	private String getGuide() {
		StringBuilder sb = new StringBuilder("");
		try (
				BufferedReader br = new BufferedReader(new FileReader("./popupFiles/Guide.png"))
			) {
			String line = br.readLine();
			while (line != null) {
				sb.append(line);
